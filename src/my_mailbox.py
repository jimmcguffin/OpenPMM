...
class MailBox:
    ...
    def clean(self): # erases  items in Folder "X", should run at start or end (or both)
        ...
        file.close()
        os.remove("OpenPMM.mail")
        os.rename("OpenPMM.mail.tmp","OpenPMM.mail")
        # self.load() # caller needs to do this if at start, if at end, no need

    def load(self):
        self.mail.clear()
        try:
            with open("OpenPMM.mail","rb") as file:
                while True:
                    oh = file.tell()
    ...
    def add_mail(self,mbh:MailBoxHeader,message:str,folder:MailFlags):
        ...
        with open("OpenPMM.mail","ab") as file:
