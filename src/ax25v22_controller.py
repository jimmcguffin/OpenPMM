...
class AX25_Controller(QObject):
    ...
    def on_t2_timeout(self):
        ...
        # if needed, send an I frame or an RR
        if self.state_connected_i_frame_queue_not_empty():
            return
        else:
            self.nr = self.vr
    ...
    def on_bytes_ready(self,msg:bytes):
        ...
        if frame.control.frame_type.is_I():
            self.ns = frame.control.send_seqno
            print(f"Timer 2 set at {datetime.now().strftime("%H:%M:%S.%f")}")
            self.t2.start()
        if not frame.control.frame_type.is_U():
            self.nr = frame.control.recv_seqno

        print(f"new packet: vs={self.vs} vr={self.vr} v={self.va} ns={self.ns} nr={self.nr}")
    ...
    def state_connected_i_frame_queue_not_empty(self):
        if not self.i_frame_queue: # queue is not supposed to be empty!
            return False
        if self.vs == self.va + self.k: # at maximum window size (backlog)
            return False
        # last count of each gets set to next of each
        self.ns = self.vs
        self.nr = self.vr
    ...
    def state_connected_message(self,msg:bytes): # this is a long one
        ...
        match ft:
            ...
            case ax25.FrameType.RR | ax25.FrameType.RNR:
                ...
                if True or self.va <= self.nr <= self.vs: # the range check works wrong in mod8
            ...
            case ax25.FrameType.I:
                if not frame.is_command(): # if frame.dst.command_response and not frame.src.command_response:
                    self.error_code = "S"
                    return AX25State.CONNECTED
                if True or self.va <= self.nr <= self.vs: # the range check works wrong in mod8
                    self.check_i_frame_ackd()
