...
class TAPR_Pipeline(Pipeline):
    def __init__(self,pd,l1:Level1):
        ...
        self.l1.signal_bytes_ready.connect(self.line_parser.on_bytes_ready)
        self.line_parser.signal_bytes_ready.connect(self.tnc_parser.on_bytes_ready)
        self.tnc_parser.signal_write.connect(self.l1.write)
    ...
    def remove_bbs(self):
        if self.bbs:
            ...
            #self.tnc_parser.signal_bytes_ready.disconnect
    ...
    def stop_session(self):
        self.remove_bbs()
        if self.tnc_parser:
            self.tnc_parser.stop_session() #! this can take a while, isn't done when this returns
            #self.tnc_parser.signal_bytes_ready.disconnect()
            self.tnc_parser = None
        if self.line_parser:
            #self.line_parser.signal_bytes_ready.disconnect()
...
class KISS_Pipeline(Pipeline):
    def __init__(self,pd,l1:Level1):
        ...
        self.ax25_controller = AX25_Controller(self)
    ...
    def stop_session(self):
        self.remove_bbs()
        if self.line_parser:
            #self.line_parser.signal_bytes_ready,self.disconnect()
            self.line_parser = None
        if self.ax25_controller:
            self.ax25_controller.stop_session()
            #self.ax25_controller.signal_bytes_ready.disconnect()
            #self.ax25_controller.signal_write.disconnect()
            self.ax25_controller = None
