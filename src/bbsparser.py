...
class BbsParser(QObject):
    ...
    def dump_sequence(self,label):
        ...
        print("Pending:")
        for s in self.bbs_pending:
            assert(isinstance(s,BbsSequenceStep))
            if isinstance(s,BbsSequenceStep):
                print(f"  step: wts={s.what_to_send.replace("\r","|")} h={s.handler} d={s.data}")
        print()
...
class Jnos2Parser(BbsParser):
    ...
    def handle_list(self,r,_=None):
        ...
        # sample "la\r\nMail area: k6gx\r\n1 message  -  1 new\r\n\St.  #  TO            FROM     DATE   SIZE SUBJECT\r\n> N   1 K6GX@w1xsc.sc pkttue   Oct 15  747 DELIVERED: W6W-303P_P_ICS213_Shutti\r\nArea: k6gx Current msg# 1.\r\n" +terminator
    ...
    def send_confirmations(self,_=None):
        ...
        for mbh in self.messages_to_be_acknowledged:
            ...
            global_signals.signal_new_outgoing_receipt.emit(mbh)
    ...
    def handle_read(self,r,data):
        ...
        if in_home_area and not mbh.subject.startswith("DELIVERED:"):
            if messagebody.find("!RDR!") >= 0 or self.pd.getProfileBool("MessageSettings/SendDeliveryReceipt"):
