...
class SerialStream(QObject):
    ...
    def find_lines(self):
        ...
        while not done:
            ...
            if (p := self._sdata.find(self.line_end,start)) >= 0:
                if self.include_line_end_in_reply:
                    self.signalLineRead.emit(self._sdata[0:p+len(self.line_end)].decode(self.encoding))
                else:
                    self.signalLineRead.emit(self._sdata[0:p].decode(self.encoding))
                # extract
        ...
        #         end = i+elen
        #         if self.include_line_end_in_reply:
        #             self.signalLineRead.emit(self._sdata[start:end].decode(self.encoding))
        #         else:
        #             self.signalLineRead.emit(self._sdata[start:end-elen].decode(self.encoding))
        #         start = end
        #         i = start
